"use strict";
    ...
    const SLOT_NAMES = ["Head", "RArm", "LArm", "Chest", "Legs"];
    const slotLabel = {Head:"Head", RArm:"R.Arm", LArm:"L.Arm", Chest:"Chest", Legs:"Legs"};
    const slotEs = {Head:"Cabeza", RArm:"Brazo D.", LArm:"Brazo I.", Chest:"Pecho", Legs:"Piernas"};
    ...
    const partTemplates = {
      ...
      "Dagger Hand": {slot:"RArm", stats:{atk:4, spd:-1}, edges:[["RArm","Chest"],["RArm","Head"]], color:"#b8ffda"},
      "Goblin Legs": {slot:"Legs", stats:{spd:3}, edges:[["Legs","Chest"]], color:"#00ff88"},
      "Demon Claw": {slot:"RArm", stats:{atk:7, def:-2}, edges:[["RArm","Chest"],["RArm","Head"]], color:"#ff2b2b"},
      "Demon Core": {slot:"Chest", stats:{hp:20}, edges:[["Chest","Head"],["Chest","Legs"],["Chest","LArm"]], color:"#8b0000"},
      "Stone Fist": {slot:"LArm", stats:{atk:10}, edges:[["LArm","Chest"],["LArm","RArm"]], color:"#b6b6a8"},
      ...
      "Kingbreaker Relic": {slot:"Chest", stats:{hp:70, atk:18, def:8, spd:10}, edges:[["Chest","Head"],["Chest","Legs"],["Chest","RArm"],["Chest","LArm"],["Head","RArm"]], color:"#ffef6e"},
      ...
      "Blood Seal": {slot:"RArm", stats:{atk:10, hp:10}, edges:[["RArm","Chest"],["RArm","Head"],["RArm","Legs"]], color:"#ff3d3d"},
    ...
    const levels = [
      {name:"Bosque Hundido", boss:"Guardian de Raiz", reward:"Root Mask", minions:3, hp:70, atk:10, speed:25, sky:["#c9dee0","#7fa6a8","#26444f","#071014"], accent:"#9fd968"},
      {name:"Templo de Niebla", boss:"Monje del Osario", reward:"Mist Tabi", minions:4, hp:115, atk:14, speed:32, sky:["#d6d9e3","#8d9aaa","#414a64","#090b16"], accent:"#b8ffda"},
      {name:"Forja Sangrienta", boss:"Herrero Demonio", reward:"Blood Seal", minions:5, hp:165, atk:19, speed:35, sky:["#d9b8a1","#8a4a3a","#33151b","#080407"], accent:"#ff3d3d"},
      {name:"Trono del Vacio", boss:"Rey del Vacio", reward:"Void Crown", minions:6, hp:235, atk:25, speed:42, sky:["#c8c5da","#71668f","#211c36","#030307"], accent:"#ffd700"}
    ];
    const keys = new Set();
    const logLines = [];
    const particles = [];
    ...
    playerSprite.img.onload = () => { playerSprite.ready = true; log("Spritesheet del jugador cargado: assets/player-spritesheet.png"); };
    playerSprite.img.onerror = () => { playerSprite.ready = false; log("Spritesheet del jugador no encontrado: usando rana ronin procedural"); };
    playerSprite.img.src = "assets/player-spritesheet.png";
    const levelScene = {img:new Image(), ready:false, x:0, y:0, w:0, h:0};
    levelScene.img.onload = () => { levelScene.ready = true; log("Escenario top-down cargado para Nivel 1"); };
    ...
    const player = {
      ...
      atk:8, def:2, spd:95, invincible:0, attack:0, shotCooldown:0, dir:{x:1,y:0}, vy:0, onGround:false, inventory:[], equipped:{Head:null,RArm:null,LArm:null,Chest:null,Legs:null},
      combos:new Set()
    };
    let wave = 1;
    ...
    function resize() {
      ...
      for (const c of [buildCanvas, mapCanvas]) {
        ...
        c.width = Math.max(240, Math.floor(r.width));
        c.height = Math.max(110, Math.floor(r.height));
      }
    }
    ...
    function recalcStats() {
      ...
      player.combos.clear();
      if (hasPath("RArm", "Legs", ["Chest"])) player.combos.add("Berserker");
    ...
    function killEnemy(e) {
      ...
      const name = e.drops[Math.floor(Math.random() * e.drops.length)];
      drops.push({x:e.x, y:groundYAt(e.x) - 10, part:makePart(name), bob:0});
      log(`${enemyEs[e.name]} derrotado: solto ${partEs[name]}`);
    }
    ...
    function damageBoss(amount) {
      ...
      if (boss.hp <= 0) {
        const dropName = boss.reward;
        const dropX = boss.x + rand(-20,20);
        const part = makePart(dropName);
        part.levelReward = true;
        drops.push({x:dropX, y:groundYAt(dropX) - 12, part, bob:0});
        log(`${boss.name} derrotado: robale ${partEs[dropName]} para abrir el siguiente nivel.`);
        boss = null;
    ...
    function drawLevelSetPieces(level) {
      ...
      if (levelState === "reward") {
        g.fillStyle = "#ffd700";
        g.fillText(`ROBA ${partEs[level.reward]} PARA AVANZAR`, w * .5, signY + 44);
    ...
    function drawPlayerSprite(x, y) {
      ...
      let row = 6;
      let frames = 8;
      let frame = Math.floor(tick / 7) % frames;
      if (airborne) {
        row = 1;
        frame = player.vy < 0 ? 0 : 4;
      } else if (player.attack > 0) {
        row = 3;
        frames = 4;
        frame = Math.min(3, Math.floor((1 - player.attack) * 4));
      } else if (moving) {
        row = 6;
        ...
        row = 7;
        frames = 4;
        frame = Math.floor(tick / 14) % frames;
      }
      const sw = playerSprite.frameW, sh = playerSprite.frameH;
      const levelScale = isTopDownLevel() ? .72 : playerSprite.scale;
      const dw = sw * levelScale, dh = sh * levelScale;
      g.save();
      g.translate(x, y + 3);
      if (player.dir.x < 0) g.scale(-1, 1);
      g.drawImage(playerSprite.img, frame * sw, row * sh, sw, sh, -dw / 2, -dh + 23, dw, dh);
      g.restore();
    }
    ...
    function drawDrop(d) {
      ...
      g.strokeStyle = "#ffd700"; g.lineWidth = 1; g.strokeRect(d.x-11,y-11,22,22);
    }
    ...
    function drawBuildGraph() {
      const cw = buildCanvas.width, ch = buildCanvas.height;
      bg.clearRect(0,0,cw,ch);
      const pos = {
        Head:{x:cw*.5,y:ch*.18}, Chest:{x:cw*.5,y:ch*.48}, LArm:{x:cw*.25,y:ch*.45}, RArm:{x:cw*.75,y:ch*.45}, Legs:{x:cw*.5,y:ch*.8}
      };
      ...
      for (const [a, edges] of buildGraph) for (const b of edges) if (a < b) {
        ...
        bg.strokeStyle = key ? "#ffd700" : "#7b4e98"; bg.lineWidth = key ? 5 : 2;
      ...
      for (const n of SLOT_NAMES) {
        ...
        bg.fillStyle = equipped ? "rgba(0,255,136,.14)" : "rgba(109,102,128,.08)";
        bg.beginPath(); bg.arc(p.x,p.y,23,0,Math.PI*2); bg.fill();
        bg.beginPath(); bg.arc(p.x,p.y,15,0,Math.PI*2);
        bg.fillStyle = equipped ? "#1a0a2e" : "#050508"; bg.fill();
        bg.strokeStyle = equipped ? "#00ff88" : "#6d6680"; bg.setLineDash(equipped ? [] : [4,4]); bg.lineWidth = 2; bg.stroke(); bg.setLineDash([]);
        bg.fillStyle = equipped ? "#e8e0d0" : "#6d6680"; bg.font = "11px Courier New"; bg.textAlign = "center";
        bg.fillText(slotEs[n], p.x, p.y + 31);
      }
      document.getElementById("adjList").textContent = "Lista de adyacencia:\n" + SLOT_NAMES.map(n => `${slotEs[n]} -> [${buildGraph.get(n).map(x => slotEs[x]).join(", ")}]`).join("\n");
    }
    ...
    function drawMapGraph() {
      ...
      const objective = awaitingBossReward
        ? `Robar: ${partEs[currentLevel().reward]}`
        : boss
    ...
    addEventListener("keydown", e => {
      ...
      if (["w","a","s","d"," ","i","e","r","1","2"].includes(k)) e.preventDefault();
      keys.add(k);
    });
    ...
    spawnWave();
    log("Graph data structures initialized: dungeonGraph + buildGraph");
    requestAnimationFrame(loop);
